...
The proposed AMLD framework consists of three main stages: magnetic signal denoising, cross-node vehicle re-identification based on EF-IMF-DTW, and vehicle speed estimation. Representative results of each stage are presented below.

### 1. AMLD-based Denoising

The proposed AMLD method suppresses interference components in the magnetic signals while preserving the main vehicle-related magnetic response. Representative upstream and downstream signals before and after denoising are shown below.
...
<p align="center">
  <img src="figure/upstream.png" width="600">
</p>

**After denoising**

<p align="center">
  <img src="figure/u_d.png" width="600">
</p>
...
<p align="center">
  <img src="figure/downstream.png" width="600">
</p>

**After denoising**

<p align="center">
  <img src="figure/d_d.png" width="600">
</p>

The original upstream and downstream magnetic signals contain noticeable interference components that may obscure the vehicle-induced magnetic variations. After AMLD-based denoising, these interference components are effectively suppressed, while the main waveform characteristics associated with the passing vehicle are well preserved. The denoised signals therefore provide more reliable features for subsequent cross-node vehicle re-identification.

---

### 2. EF-IMF-DTW-based Vehicle Re-identification

After denoising, vehicle re-identification between the upstream and downstream sensor nodes is performed using the proposed EF-IMF-DTW method. The matching results for the upstream and downstream vehicle signals are illustrated below.

#### Upstream Signal

<p align="center">
  <img src="figure/DTW_u.png" width="600">
</p>

#### Downstream Signal

<p align="center">
  <img src="figure/DTW_d.png" width="600">
</p>

EF-IMF-DTW compares the IMF-based features of vehicle magnetic signals collected at the two sensor nodes and evaluates their similarity through energy-weighted DTW distances. By emphasizing informative IMF components, the method reduces the influence of residual interference and waveform distortion on cross-node matching. The matched upstream and downstream signals are subsequently regarded as responses generated by the same vehicle.

---

### 3. Vehicle Speed Estimation

Once the same vehicle is successfully matched between the upstream and downstream sensor nodes, its speed is estimated according to the spatial distance between the two sensors and the corresponding time delay.

For two sensor nodes separated by a known distance \(D\), the estimated vehicle speed is calculated as

\[
\hat{v} = \frac{D}{\Delta t},
\]

where \(\Delta t\) denotes the time interval between the matched vehicle responses at the upstream and downstream sensor nodes.

Therefore, the complete AMLD framework progressively performs interference suppression, cross-node vehicle re-identification, and speed estimation. Reliable denoising improves the discriminability of vehicle magnetic signatures, while EF-IMF-DTW establishes the correspondence between upstream and downstream observations, providing the basis for accurate vehicle speed estimation.
